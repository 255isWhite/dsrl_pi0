...
def update_res_actor(key: PRNGKey, res_actor: TrainState, clean_critic: TrainState, 
    ...
    def res_actor_loss_fn(
        ...
        if hasattr(res_actor, 'batch_stats') and res_actor.batch_stats is not None:
            res_actions, new_model_state = res_actor.apply_fn({'params': res_actor_params, 'batch_stats': res_actor.batch_stats}, batch['observations'], mutable=['batch_stats'])
        else: 
            res_actions = res_actor.apply_fn({'params': res_actor_params}, batch['observations'])
            new_model_state = {}
