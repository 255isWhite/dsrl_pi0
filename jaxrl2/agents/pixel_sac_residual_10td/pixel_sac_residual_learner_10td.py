...
from jaxrl2.agents.pixel_sac_residual_10td.actor_updater import update_actor, update_res_actor
from jaxrl2.agents.pixel_sac_residual_10td.critic_updater import update_critic, update_critic_wo_actor, update_clean_critic, update_clean_critic_wo_actor
from jaxrl2.agents.pixel_sac_residual_10td.temperature_updater import update_temperature
from jaxrl2.agents.pixel_sac_residual_10td.temperature import Temperature
...
def _update_jit(
    ...
    res_actor: TrainState, clean_critic: TrainState,
    target_critic_params: Params, clean_target_critic_params: TrainState, temp: TrainState, batch: TrainState,
    discount: float, tau: float, target_entropy: float,
    critic_reduction: str, color_jitter: bool, aug_next: bool, num_cameras: int,
    kl_coeff: float, res_coeff: float, td3_noise_scale: float
) -> Tuple[PRNGKey, TrainState, TrainState, Params, TrainState, Dict[str,float]]:
    print(f"[JIT Compile] sac learner update jit")
    # # 🚨 打印所有静态参数
    # print("[JIT Compile Triggered] --------------------")
    # print("critic_reduction:", critic_reduction, type(critic_reduction))
    # print("color_jitter:", color_jitter, type(color_jitter))
    # print("aug_next:", aug_next, type(aug_next))
    # print("num_cameras:", num_cameras, type(num_cameras))

    # # 🚨 打印 batch 的结构/shape/dtype
    # def _print_tree(tree, prefix="batch"):
    #     if isinstance(tree, (FrozenDict, dict)):
    ...
    # # 🚨 打印 actor/critic 的 param 树结构
    # def _print_params(name, params):
    #     if hasattr(params, "keys"):
    #         print(f"{name} param tree keys:", list(params.keys()))
    #     else:
    #         print(f"{name} type:", type(params))

    # _print_params("actor", actor.params if hasattr(actor, "params") else actor)
    # _print_params("critic", critic.params if hasattr(critic, "params") else critic)
    # _print_params("res_actor", res_actor.params if hasattr(res_actor, "params") else res_actor)
    # _print_params("clean_critic", clean_critic.params if hasattr(clean_critic, "params") else clean_critic)
    ...
    new_clean_critic, clean_critic_info = update_clean_critic(
        key, clean_critic, clean_target_critic, res_actor, batch, discount, res_coeff, critic_reduction=critic_reduction
    )
    # new_clean_critic = clean_critic
    # clean_critic_info = {}
...
def _update_jit_wo_actor(
    rng: PRNGKey, actor: TrainState, critic: TrainState,
    res_actor: TrainState, clean_critic: TrainState,
    target_critic_params: Params, clean_target_critic_params: TrainState, batch: TrainState,
    discount: float, tau: float, target_entropy: float,
    critic_reduction: str, color_jitter: bool, aug_next: bool, num_cameras: int,
    res_coeff: float, td3_noise_scale: float
) -> Tuple[PRNGKey, TrainState, TrainState, Params, TrainState, Dict[str,float]]:
    print(f"[JIT Compile] sac learner update jit without actor")
    aug_pixels = batch['observations']['pixels']
    aug_next_pixels = batch['next_observations']['pixels']
    if batch['observations']['pixels'].squeeze().ndim != 2:
        rng, key = jax.random.split(rng)
    ...
    key, rng = jax.random.split(rng)
    clean_target_critic = clean_critic.replace(params=clean_target_critic_params)
    new_clean_critic, clean_critic_info = update_clean_critic_wo_actor(
        key, clean_critic, clean_target_critic, res_actor, batch, discount, res_coeff, critic_reduction=critic_reduction
    )
    new_clean_target_critic_params = soft_target_update(new_clean_critic.params, clean_target_critic_params, tau)

    return rng, new_critic, new_target_critic_params, new_clean_critic, new_clean_target_critic_params, {
        **critic_info,
    }, {
        **clean_critic_info
    }
...
class PixelSACResidualLearner10TD(Agent):
    ...
    def __init__(self,
        ...
        self.critic_reduction = str(critic_reduction)
        ...
        # residual head part
        res_policy_def = DeterministicPolicy(hidden_dims, self.action_dim, dropout_rate=dropout_rate, action_magnitude=action_magnitude)
        res_actor_def = TimePixelMultiplexer(encoder=encoder_def,
        ...
        print(res_actor_def)
        print("================================================================")
        res_actor_def_init = res_actor_def.init(res_actor_key, observations, times=times)
        ...
        res_actor = TrainState.create(apply_fn=res_actor_def.apply,
                                  params=res_actor_params,
                                  tx=optax.adam(learning_rate=actor_lr),
                                  batch_stats=res_actor_batch_stats)

        clean_critic_def = StateActionEnsemble(hidden_dims, num_qs=num_qs)
        clean_critic_def = TimePixelMultiplexer(encoder=encoder_def,
                                      network=clean_critic_def,
                                      latent_dim=latent_dim,
                                      use_bottleneck=use_bottleneck
                                      )
        print("=========================== clean critic def ===========================")
        print(clean_critic_def)
        print("================================================================")
        clean_critic_def_init = clean_critic_def.init(clean_critic_key, observations, actions, times)
        self._critic_init_params = clean_critic_def_init['params']

        clean_critic_params = clean_critic_def_init['params']
        clean_critic_batch_stats = clean_critic_def_init['batch_stats'] if 'batch_stats' in clean_critic_def_init else None
        clean_critic = TrainState.create(apply_fn=clean_critic_def.apply,
                                   params=clean_critic_params,
                                   tx=optax.adam(learning_rate=critic_lr),
                                   batch_stats=clean_critic_batch_stats
                                   )
        clean_target_critic_params = copy.deepcopy(clean_critic_params)
    ...
    def update(self, batch: FrozenDict) -> Dict[str, float]:
        new_rng, new_actor, new_critic, new_target_critic, new_res_actor, new_clean_critic, new_clean_target_critic, new_temp, info, res_info = _update_jit(
            self._rng, self._actor, self._critic, self._res_actor, self._clean_critic,
            self._target_critic_params, self._clean_target_critic_params, self._temp,
            batch, self.discount, self.tau, self.target_entropy, self.critic_reduction,
            self.color_jitter, self.aug_next, self.num_cameras, 
            self.kl_coeff, self.res_coeff, self.td3_noise_scale
        )
        ...
        self._temp = new_temp
        self._res_actor = new_res_actor
        self._clean_critic = new_clean_critic
        self._clean_target_critic_params = new_clean_target_critic
        return info, res_info

    def update_wo_actor(self, batch: FrozenDict) -> Dict[str, float]:
        new_rng, new_critic, new_target_critic, new_clean_critic, new_clean_target_critic, info, res_info = _update_jit_wo_actor(
            self._rng, self._actor, self._critic, self._res_actor, self._clean_critic,
            self._target_critic_params, self._clean_target_critic_params, batch, self.discount,
            self.tau, self.target_entropy, self.critic_reduction, self.color_jitter, self.aug_next, self.num_cameras, 
            self.res_coeff, self.td3_noise_scale
            )
