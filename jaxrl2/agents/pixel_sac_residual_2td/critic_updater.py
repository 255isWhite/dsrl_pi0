...
def update_critic(
        ...
        target_critic: TrainState, clean_critic: TrainState, res_actor: TrainState,
    ...
    residual_actions, raw_means = res_actor.apply_fn({'params': res_actor.params}, batch['observations'])
    dist, means, log_stds = actor.apply_fn({'params': actor.params}, batch['observations'])
    noise, log_probs = dist.sample_and_log_prob(seed=key)
    distill_clean_qs = clean_critic.apply_fn({'params': clean_critic.params},
                                            batch['observations'], residual_actions, noise)

    if critic_reduction == 'min':
        target_q = distill_clean_qs.min(axis=0)
    elif critic_reduction == 'mean':
        target_q = distill_clean_qs.mean(axis=0)
    else:
        raise NotImplemented()

    if backup_entropy:
        target_q -= batch["discount"] * batch['masks'] * temp.apply_fn(
            {'params': temp.params}) * log_probs
    ...
    def critic_loss_fn(
        ...
        return critic_loss, {
            ...
            'q': qs.mean(),
            'target_actor_entropy': -log_probs.mean(),
            # 'next_actions_sampled': next_actions.mean(),
            'next_log_probs': log_probs.mean(),
            'next_q_pi': qs.mean(),
...
def update_clean_critic(
        ...
        actor: TrainState, batch: DatasetDict, discount: float, res_coeff: float, res_prob: float, 
    ...
    batch_size = next_res_actions.shape[0]
    # old norm format
    ...
    next_dist, means, log_stds = actor.apply_fn({'params': actor.params}, batch['next_observations'])
    next_noise, log_probs = next_dist.sample_and_log_prob(seed=key_act)
    next_qs = target_clean_critic.apply_fn({'params': target_clean_critic.params},
                                     batch['next_observations'], next_res_actions, next_noise)   
    ...
    def clean_critic_loss_fn(
        ...
        clean_qs = clean_critic.apply_fn({'params': clean_critic_params}, 
                                         batch['observations'], batch['residual_actions'].reshape(batch_size,-1), batch['actions'].reshape(batch_size,-1))
        ...
        return clean_critic_loss, {
            ...
            'res_actions_mean': next_res_actions.mean(axis=0),
            'res_actions_std': next_res_actions.std(axis=0),
            'res_actions_min': next_res_actions.min(axis=0),
            'res_actions_max': next_res_actions.max(axis=0),
        }
