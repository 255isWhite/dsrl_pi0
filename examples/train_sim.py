...
class DummyEnv(gym.ObservationWrapper):
    ...
    def __init__(self, variant):
        ...
        self.action_space = Box(low=-1, high=1, shape=(50, 32,), dtype=np.float32) # 32 is the noise action space of pi 0
...
def main(variant):
    ...
    if variant.algorithm == 'pixel_sac':
        ...
        agent = PixelSACLearner(variant.seed, sample_obs, sample_action, kl_coeff=variant.kl_coeff, decay_kl=variant.decay_kl, chunk_len=variant.query_freq, distill_action_dim=distill_action_dim, distill_hidden_dim=variant.distill_hidden_dim, bc_coeff=variant.bc_coeff, **kwargs)
    ...
    online_replay_buffer = ReplayBuffer(dummy_env.observation_space, dummy_env.action_space, int(online_buffer_size), variant.query_freq, distill_action_dim)
