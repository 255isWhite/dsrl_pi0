...
def main(variant):
    ...
    kwargs['expectile'] = variant.expectile
    ...
    if variant.algorithm == 'pixel_sac':
        ...
        agent = PixelSACLearner(variant.seed, sample_obs, sample_action, sample_times, \
            kl_coeff=variant.kl_coeff, decay_kl=variant.decay_kl , \
                guidance_scale=variant.guidance_scale, **kwargs)
    elif variant.algorithm == 'pixel_sac_iql_single_v':
        print("Using pixel sac iql single v learner")
        agent = PixelSACIQLSingleVLearner(variant.seed, sample_obs, sample_action, sample_times, \
            kl_coeff=variant.kl_coeff, decay_kl=variant.decay_kl , \
                guidance_scale=variant.guidance_scale, **kwargs)
