...
def add_online_data_to_buffer(variant, traj, online_replay_buffer):
    ...
    residual_actions = np.array(traj['residual_actions']) # (T, chunk_size, action_dim )
    ...
    for t in range(episode_len):
        ...
        insert_dict = dict(
            ...
            residual_actions=residual_actions[t],
            rewards=rewards[t],
            masks=masks[t],
...
def collect_traj(variant, agent, env, i, agent_dp=None, res_prob=0.0, dp_unnorm_transform=None, use_random=False):
    ...
    for t in tqdm(range(max_timesteps)):
        ...
        if t % query_frequency == 0:
            ...
            # b.
            if use_residual:
                residual_actions = agent.sample_residual_actions(obs_dict, action_real_dim).squeeze(0)
                actions = action_dict["norm_actions"][:query_frequency]+ res_coeff * residual_actions
                actual_norm_action = actions.copy()
                actions = dp_unnorm_transform({'actions': actions})['actions']
            else:
                residual_actions = np.zeros((query_frequency, action_real_dim))
    ...
    return {
        ...
        'residual_actions': residual_actions_list,
        'rewards': rewards,
        'masks': masks,
